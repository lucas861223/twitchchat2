...
class test:
    MESSAGE_PATTERN = re.compile(
        '@badges=([^;]*);.*(bits=(\d+);.*)?color=([^;]*);.*display-name=(([^A-Za-z]*)|([^;]*));.*emotes=([^;]*);.*user-id=(\d+);.*:([^!]+)!.*#[^ ]+ :(ACTION )?(.*)')
    def __init__(self, chatScreen):
        file = open('C:\\Users\\lucas\\Python Workspace\\TwitchChat\\src\\setting\\login', 'r')
        ...
        while self.receiveSocketRunning:
            try:
                ...
                for responses in response.split('\r\n'):
                    message = re.search(test.MESSAGE_PATTERN, responses)
                    if message.group(7) == "StreamElements":
                        pass
